...
public class CosmeticItemRenderLayer implements LayerRenderer<EntityPlayer> {
	...
	@Override
	public void doRenderLayer(@Nonnull EntityPlayer player, float limbSwing, float limbSwingAmount, float partialTicks,
	...
	private void greenHats(EntityPlayer player, float partialTicks) {
		...
		GlStateManager.pushMatrix();
		int hats = player.isPotionActive(ModPotions.vegetable)
				? player.getActivePotionEffect(ModPotions.vegetable).getAmplifier() + 1
				: 0;
		...
		/**
		 * Helper.translateToHeadLevel(player); Helper.translateToFace();
		 * Helper.defaultTransforms(); GlStateManager.scale(1.4F, 1.6F, 1.4F);
		 * GlStateManager.translate(0F, 0.35F * i, 0F); GlStateManager.translate(-0.01F
		 * * i, 0F, 0F); TextureAtlasSprite gemIcon =
		 * MiscellaneousIcons.INSTANCE.greenhatIcon; float f = gemIcon.getMinU(); float
		 * f1 = gemIcon.getMaxU(); float f2 = gemIcon.getMinV(); float f3 =
		 * gemIcon.getMaxV(); IconHelper.renderIconIn3D(Tessellator.getInstance(), f1,
		 * f2, f, f3, gemIcon.getIconWidth(), gemIcon.getIconHeight(), 1F / 32F);
		 **/
		...
		Minecraft.getMinecraft().renderEngine
				.bindTexture(new ResourceLocation("extrabotany:textures/model/greenhat.png"));
		GlStateManager.pushMatrix();
		if (hats > 0)
	...
	private void dispatchRenders(EntityPlayer player, RenderType type, float partialTicks) {
		...
		for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
			...
			if (stack.getItem() instanceof ICosmeticItem && player.inventory.hasItemStack(stack)
					&& !player.getHeldItemMainhand().isItemEqual(stack)) {
				GlStateManager.pushMatrix();
				GL11.glColor3ub((byte) 255, (byte) 255, (byte) 255);
				GlStateManager.color(1F, 1F, 1F, 1F);
			...
			if (stack.getItem() instanceof ItemBaubleBox && player.inventory.hasItemStack(stack)
					&& player.getName() == "ExtraMeteorP") {
				...
				for (int s = 0; s < newInv.getSlots(); s++) {
					if (newInv.getStackInSlot(s).getItem() instanceof IBaubleRender) {
						...
						GL11.glColor3ub((byte) 255, (byte) 255, (byte) 255);
						GlStateManager.color(1F, 1F, 1F, 1F);
						((IBaubleRender) newInv.getStackInSlot(s).getItem())
								.onPlayerBaubleRender(newInv.getStackInSlot(s), player, type, partialTicks);
						GlStateManager.popMatrix();
					}
				}
			}
