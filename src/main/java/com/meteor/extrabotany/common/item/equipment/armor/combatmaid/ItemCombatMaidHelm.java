...
public class ItemCombatMaidHelm extends ItemCombatMaidArmor implements IManaDiscountArmor, IManaGivingItem, IManaUsingItem{
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
		...
		if(hasArmorSet(player)) {
			...
			if(player.shouldHeal() && player.ticksExisted % 50 == 0 && ManaItemHandler.requestManaExactForTool(stack, player, 20, true))
				player.heal(1F);
	...
	public void onEntityAttacked(LivingHurtEvent event) {
		...
		if(attacker instanceof EntityPlayer) {
			...
			if(hasArmorSet(player)) {
				if(player.getHeldItemMainhand() == ItemStack.EMPTY && ManaItemHandler.requestManaExactForTool(new ItemStack(Items.APPLE), player, 250, true))
					event.setAmount(event.getAmount() + 10F);
				if(player.shouldHeal() && ManaItemHandler.requestManaExactForTool(new ItemStack(Items.APPLE), player, 100, true))
