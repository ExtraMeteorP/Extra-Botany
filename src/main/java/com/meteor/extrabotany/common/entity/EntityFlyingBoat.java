...
public class EntityFlyingBoat extends Entity {
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager
			.<Integer>createKey(EntityFlyingBoat.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager
			.<Integer>createKey(EntityFlyingBoat.class, DataSerializers.VARINT);
	private static final DataParameter<Float> RAFT_DAMAGE_TAKEN = EntityDataManager
			.<Float>createKey(EntityFlyingBoat.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> RAFT_TYPE = EntityDataManager.<Integer>createKey(EntityFlyingBoat.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {
			EntityDataManager.createKey(EntityFlyingBoat.class, DataSerializers.BOOLEAN),
			EntityDataManager.createKey(EntityFlyingBoat.class, DataSerializers.BOOLEAN) };
	...
	/**
	 * How much the raft should glide given the slippery blocks it's currently
	 ...
	 * blocks. This enables the entity to be pushable on contact, like rafts or
	 * minecarts.
	 */
	@Override
	@Nullable
	...
	/**
	 * Decides how much the raft should be gliding on the land (based on any
	 * slippery blocks)
	 */
	...
	private void updateMotion() {
		...
		if (this.previousStatus == EntityFlyingBoat.Status.IN_AIR && this.status != EntityFlyingBoat.Status.IN_AIR
				&& this.status != EntityFlyingBoat.Status.ON_LAND) {
	...
	/**
	 * Applies this raft's yaw to the given entity. Used to update the orientation
	 * of its passenger.
	 */
						...
						if (this.world.getGameRules().getBoolean("doEntityDrops")) {
							for (int i = 0; i < 3; ++i) {
								this.entityDropItem(new ItemStack(Item.getItemFromBlock(Blocks.PLANKS), 1,
										this.getRaftType().getMetadata()), 0.0F);
							}
	 ...
	 * "drives" the vehicle. For example, Pigs, Horses, and Rafts are generally
	...
	public static enum Type {
		MANASTEEL(0, "manasteel"), ELEMENTIUM(1, "elementium"), TERRASTEEL(2, "terrasteel");
