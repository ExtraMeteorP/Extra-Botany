...
public class LensSmelt extends Lens {
	...
	public boolean collideBurst(IManaBurst burst, EntityThrowable entity, RayTraceResult pos, boolean isManaBlock,
			boolean dead, ItemStack stack) {
		...
		if (!source.equals(pos.getBlockPos()) && !(tile instanceof IManaBlock) && neededHarvestLevel <= harvestLevel
				...
				if (!burst.isFake()) {
					...
					if (!itemstack.isEmpty()) {
						if (!entity.world.isRemote) {
							...
							if (xp >= 1F) {
								entity.world.getBlockState(pos.getBlockPos()).getBlock()
										.dropXpOnBlockBreak(entity.world, pos.getBlockPos(), (int) xp);
							}
							...
							if (ConfigHandler.blockBreakParticles) {
								...
								for (int i = 0; i < 3; i++)
									entity.world.spawnParticle(EnumParticleTypes.FLAME,
											...
											pos.getBlockPos().getY() + Math.random() - 0.5F,
											pos.getBlockPos().getZ() + Math.random() - 0.5F, 0.0D, 0.0D, 0.0D);
							}
						}
