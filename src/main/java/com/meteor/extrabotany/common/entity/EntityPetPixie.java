...
public class EntityPetPixie extends EntityFlying{
	...
	private static final DataParameter<Integer> TYPE = EntityDataManager.createKey(EntityPetPixie.class, DataSerializers.VARINT);
	...
	public void onEntityUpdate() {
		...
		List<EntityLivingBase> livings = world.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(this.getPosition().add(-RANGE, -RANGE, -RANGE), this.getPosition().add(RANGE + 1, RANGE + 1, RANGE + 1)));
		...
		if(world.isRemote){
			...
			Botania.proxy.sparkleFX(posX + (Math.random() - 0.5) * 0.25, posY + 0.5  + (Math.random() - 0.5) * 0.25, posZ + (Math.random() - 0.5) * 0.25, 0.08F, 1F, 0.08F, 0.01F + (float) Math.random() * 0.25F, 20);
		}
	...
	public void setDead() {
		if(world != null && world.isRemote)
			for(int i = 0; i < 12; i++)
				Botania.proxy.sparkleFX(posX + (Math.random() - 0.5) * 0.25, posY + 0.5  + (Math.random() - 0.5) * 0.25, posZ + (Math.random() - 0.5) * 0.25, 0.08F, 1F, 0.08F, 1F + (float) Math.random() * 0.25F, 5);
		super.setDead();
	}
