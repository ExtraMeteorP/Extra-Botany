...
public class RenderSplashGrenade extends Render<EntitySplashGrenade> {
	
	public RenderSplashGrenade(RenderManager renderManager) {
		super(renderManager);
	}

	@Override
	public void doRender(@Nonnull EntitySplashGrenade entity, double x, double y, double z, float par8, float par9) {
		GlStateManager.pushMatrix();
        this.bindEntityTexture(entity);
        GlStateManager.translate((float)x, (float)y, (float)z);
        GlStateManager.enableRescaleNormal();
        TextureAtlasSprite textureatlassprite = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getParticleIcon(entity.getPotion().getItem(), entity.getPotion().getMetadata());
	...
	@Nonnull
	@Override
	protected ResourceLocation getEntityTexture(@Nonnull EntitySplashGrenade entity) {
