...
public class ModPetalRecipe {
	...
	public static void init() {
		if (ConfigHandler.ENABLE_BE)
			bloodyenchantressRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_BLOODYENCHANTRESS), red, red, red, red,
					runeFire, runeSummer, runeWrath);
		if (ConfigHandler.ENABLE_MB)
			moonblessRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_MOONBLESS), red, red, red, white);
		if (ConfigHandler.ENABLE_SB)
			sunblessRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_SUNBLESS), yellow, yellow, yellow, white);
		if (ConfigHandler.ENABLE_OV)
			omnivioletRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_OMINIVIOLET), blue, blue, purple, purple,
					runeSpring, runeMana, runeLust);
		if (ConfigHandler.ENABLE_SS)
			stonesiaRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_STONESIA), gray, gray, black, gaiaSpirit,
		...
		if (ConfigHandler.ENABLE_BF)
			bellflowerRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_BELLFLOWER), yellow, yellow, lime, lime,
					new ItemStack(ModItems.material));
		if (ConfigHandler.ENABLE_AF)
			annoyingflowerRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_ANNOYINGFLOWER), white, white, pink, pink,
					green, runeMana, new ItemStack(ModItems.material));
		if (ConfigHandler.ENABLE_SL)
			stardustlotusRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_STARDUSTLOTUS), purple, purple, purple, magenta,
					magenta, runeEnvy, runePride, gaiaSpirit);
		if (ConfigHandler.ENABLE_ML)
			manalinkiumRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_MANALINKIUM), lightBlue, lightBlue, cyan, cyan,
					cyan, runeLust, runeSloth, gaiaSpirit);
		if (ConfigHandler.ENABLE_RL)
			reikarlilyRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_REIKARLILY), lightBlue, lightBlue, cyan, cyan,
					blue, runePride, runeSloth, runeEnvy, gaiaSpirit);
		if (ConfigHandler.ENABLE_EO)
			enchantedorchidRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_ENCHANTEDORCHID), purple, purple, magenta, lime,
					lime, runePride, runeGreed, runeGluttony, gaiaSpirit);
		if (ConfigHandler.ENABLE_EW)
			edelweissRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_EDELWEISS), runeWinter, runeMana, white, white,
					white, lightBlue, lightBlue, manaPowder);
		if (ConfigHandler.ENABLE_MT)
			mirrortuniaRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_MIRROWTUNIA), runeAir, runePride, runeWrath,
					cyan, cyan, lightBlue, blue, manaPowder);
		if (ConfigHandler.ENABLE_GO)
			geminiorchidRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_GEMINIORCHID), orange, orange, yellow, yellow,
					orange, yellow, manaPowder, manaPowder);
		if (ConfigHandler.ENABLE_NF)
			necrofleurRecipe = BotaniaAPI.registerPetalRecipe(
					ItemBlockSpecialFlower.ofType(LibBlocksName.SUBTILE_NECROFLEUR), gray, gray, red, pink, pink,
					manaPowder, runeWrath);
	}
