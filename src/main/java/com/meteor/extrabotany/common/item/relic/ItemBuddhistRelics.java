...
public class ItemBuddhistRelics extends ItemModRelic implements IManaUsingItem{
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
		...
		if (slot == slot.MAINHAND) {
			attrib.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(uuid, "Weapon modifier", getMode(stack) == 4 ? 15 : 0, 0));
			attrib.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(uuid, "Weapon modifier" , getMode(stack) == 4 ? 0.3 : 0, 1));
		}
		return attrib;
	}
	...
	public void leftClick(PlayerInteractEvent.LeftClickEmpty evt) {
		if (!evt.getItemStack().isEmpty()
				&& evt.getItemStack().getItem() == this && getMode(evt.getItemStack()) == 4) {
	...
	public void trySpawnBurst(EntityPlayer player) {
		if (!player.getHeldItemMainhand().isEmpty()
				&& player.getHeldItemMainhand().getItem() == this && getMode(player.getHeldItemMainhand()) == 4) {
		...
		if(getMode(stack) == 2){
        	if(!world.isRemote) {
    			for(PotionEffect effect : ModBrew.oneforall.getPotionEffects(stack)) {
    				PotionEffect newEffect = new PotionEffect(effect.getPotion(), (int)((float)effect.getDuration() * 2), effect.getAmplifier() + 1, true, true);
    			...
    			if(world.rand.nextBoolean())
    				world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_BURP, SoundCategory.PLAYERS, 1F, 1F);
        	}
        }
