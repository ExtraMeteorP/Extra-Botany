		...
		if(ticksExisted >= 55) {
			...
			if(!world.isRemote) {
				...
				for(EntityPlayer player : players) {
					player.attackEntityFrom(DamageSource.causeIndirectMagicDamage(this, summoner), 4);
					if(getType() == 1){
						ExtraBotanyAPI.dealTrueDamage(player, 25F);
					...
					player.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 70, 5));
					PotionEffect wither = new PotionEffect(MobEffects.WITHER, 100, 2);
