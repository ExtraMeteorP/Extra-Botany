...
public class ItemCombatMaidArmor extends ItemCosmeticMaidArmor {
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
		...
		if (slot == armorType) {
			attrib.put(SharedMonsterAttributes.KNOCKBACK_RESISTANCE.getName(),
					new AttributeModifier(uuid, "Combatmaid modifier " + type,
							(double) getArmorDisplay(null, new ItemStack(this), type.getIndex()) / 20, 0));
			attrib.put(SharedMonsterAttributes.MAX_HEALTH.getName(),
					new AttributeModifier(uuid, "Combatmaid modifier " + type, night ? 15 : 5, 0));
			attrib.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(),
					new AttributeModifier(uuid, "Combatmaid modifier " + type, night ? 0.3F : 0, 1));
			attrib.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(),
					new AttributeModifier(uuid, "Combatmaid modifier " + type, night ? 1F : 0, 1));
			attrib.put(SharedMonsterAttributes.ATTACK_SPEED.getName(),
					new AttributeModifier(uuid, "Combatmaid modifier " + type, night ? 0.2F : 0, 1));
		}
		return attrib;
	}
	...
	public ItemStack[] getArmorSetStacks() {
		if (armorset == null)
			armorset = new ItemStack[] { new ItemStack(ModItems.cmhelm), new ItemStack(ModItems.cmchest),
					new ItemStack(ModItems.cmleg), new ItemStack(ModItems.cmboot) };
