...
public class TileManaBarrel extends TileEntity implements IManaPool, ITickable, IThrottledPacket{
	...
	public void update() {
		...
		for(BlockPos o : POOL_LOCATIONS)//input
			...
			}else if(world.getTileEntity(pos.add(o)) instanceof TileManaBarrel){
				TileManaBarrel p = (TileManaBarrel) world.getTileEntity(pos.add(o));
				if(p.getCurrentMana() >= speed && getCurrentMana() < this.manaCap){
					int current = Math.min(speed, this.manaCap - getCurrentMana());
					p.recieveMana(-current);
					recieveMana(current);
