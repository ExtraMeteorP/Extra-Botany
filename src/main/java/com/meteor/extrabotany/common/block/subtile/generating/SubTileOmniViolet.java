...
public class SubTileOmniViolet extends SubTileGenerating {
	...
	public void onUpdate() {
		...
		if (burnTime > 0) {
			burnTime--;
			mana += ConfigHandler.EFF_OMNIVIOLET * Math.min(7, buff);
		}
		...
		if (linkedCollector != null) {
			if (burnTime == 0) {
				if (mana < getMaxMana()) {
					...
					for (EntityItem item : supertile.getWorld().getEntitiesWithinAABB(EntityItem.class,
						...
						if (item.ticksExisted >= 59 + slowdown && !item.isDead) {
							...
							if (burnTime > 0 && stack.getCount() > 0) {
								...
								stack.shrink(1);
								supertile.getWorld().playSound(null, supertile.getPos(), ModSounds.endoflame,
	...
	public float bookCases() {
		...
		for (int j = -1; j <= 1; j++) {
			for (int k = -1; k <= 1; k++) {
				if (((j != 0) || (k != 0))
						&& (supertile.getWorld()
								.isAirBlock(new BlockPos(supertile.getPos().getX() + k, supertile.getPos().getY(),
										supertile.getPos().getZ() + j)))
						&& (supertile.getWorld().isAirBlock(new BlockPos(supertile.getPos().getX() + k,
								supertile.getPos().getY() + 1, supertile.getPos().getZ() + j)))) {
					temp += ForgeHooks.getEnchantPower(supertile.getWorld(),
							new BlockPos(supertile.getPos().getX() + k * 2, supertile.getPos().getY(),
									supertile.getPos().getZ() + j * 2));
					temp += ForgeHooks.getEnchantPower(supertile.getWorld(),
							new BlockPos(supertile.getPos().getX() + k * 2, supertile.getPos().getY() + 1,
									supertile.getPos().getZ() + j * 2));
					...
					if ((k != 0) && (j != 0)) {
						...
						temp += ForgeHooks.getEnchantPower(supertile.getWorld(),
								new BlockPos(supertile.getPos().getX() + k * 2, supertile.getPos().getY() + 1,
										supertile.getPos().getZ() + j));
						temp += ForgeHooks.getEnchantPower(supertile.getWorld(),
								new BlockPos(supertile.getPos().getX() + k, supertile.getPos().getY(),
										supertile.getPos().getZ() + j * 2));
						temp += ForgeHooks.getEnchantPower(supertile.getWorld(),
								new BlockPos(supertile.getPos().getX() + k, supertile.getPos().getY() + 1,
										supertile.getPos().getZ() + j * 2));
					}
				}
			}
