...
public class PersistentVariableHandler {
	...
	public static void save() throws IOException {
		...
		/*
		 * try { URL url = new URL(
		 * "https://raw.githubusercontent.com/ExtraMeteorP/RandomThings/master/advertisement.md"
		 * ); BufferedReader r = new BufferedReader(new
		 * InputStreamReader(url.openStream())); String str; while ((str = r.readLine())
		 * != null) { advertisements.add(str); } r.close(); } catch (Exception e) {
		 * e.printStackTrace(); } int countads = advertisements.size();
		 * cmp.setInteger(TAG_ADVERTISEMENT_COUNT, countads); NBTTagCompound adsCmp =
		 * new NBTTagCompound(); for (int i = 0; i < countads; i++) { NBTTagCompound
		 * adCmp = new NBTTagCompound(); adCmp.setString(TAG_ADVERTISEMENT_PREFIX,
		 * advertisements.get(i)); adsCmp.setTag(TAG_ADVERTISEMENT_PREFIX + i, adCmp); }
